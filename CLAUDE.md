...
node bin/pixegen.js fix ./upscaled.png -o ./fixed.png --colors auto  # Offline: detect native pixel grid, downscale back to it, fix colors
...
Core module map: `raster.js` (crop/blit primitives + `autoCropSubject` — noise-robust crop-to-subject before downscale, the eval loop's biggest quality lever; single sprites only), `color.js` (OKLAB), `quantize.js` (downscale/quantize/dither/outline/cleanup, incl. `downscaleKCentroid` — a noise-robust per-tile k-means downscale ported from Astropulse/pixeldetector, MIT — `estimateColorCount`'s elbow method, and `quantizeKMeans` for free-palette N-color reduction; all three share a private `kMeansRGB` helper with deterministic farthest-point/greedy-k-center seeding), `gridsize.js` (`detectPixelGrid` + `findPeaks` — recovers the native pixel size of an upscaled/JPEG-softened image via per-axis edge-energy peak-finding, also ported from pixeldetector), `preprocess.js` (denoise/sharpen/contrast presets), `palettes.js` (profiles + scales), `models.js` (static model fallback list + capability flags incl. `maxReferenceImages`), `model-registry.js` (live+curated catalog merge, `auto`/`auto:<tier>` resolution — Node fetch/cache side in `node/live-models.js`, synced by `pixegen models --live`), `prompts.js` (structured prompt spec — `buildPromptSpec`/`renderPromptSpec` compose ordered sections (format/style/subject/view/action/background) that the `buildPrompt`/`buildSheetPrompt`/`buildGridPrompt` wrappers render; also batch planning and the request-URL builder with capability gating incl. the `image=` reference param; a character's locked `styleNotes` feeds the style section), `pipeline.js` (`processSourceRaster` + sheet/grid slicing; `processSourceRaster`'s `downscale` option — `'mode' | 'average' | 'k-centroid'` — defaults to each pipeline's traditional strategy unless set explicitly, so output is byte-identical unless a caller opts in; `DOWNSCALE_OPTIONS` lists it for UI pickers next to `PIPELINE_MODES`/`DITHER_OPTIONS`), `recipes.js` (named settings bundles — the primary CLI/MCP interface; recipes may set `downscale`), `validate.js` (post-generation checks — grid-size disagreement between the AI source and the requested sprite size is a separate advisory-only signal logged by `src/node/generate.js`, not a pass/fail check here), `exporters.js` (shared sheet model — multi-animation multi-row packing with per-frame durations and pivots — plus pure emitters for Phaser/Aseprite JSON/Godot `.tres`/love2d `.lua`/Unity metadata behind the `EXPORT_FORMATS` registry; `atlas.js` is now a thin single-animation Phaser wrapper over it), `prompt-ideas.js` (random "inspired by <classic game of the selected system>" prompt generator behind the UI's 🎲 buttons and `pixegen idea`).

`pixegen fix <input> -o <out.png>` (`bin/pixegen.js`, also the `fix_pixel_art` MCP tool) is offline pixel-art repair — Retro Diffusion Pixel Art Fixer parity (`docs/RETRO-DIFFUSION.md`): decode PNG/JPEG via `decodeImage` → `detectPixelGrid` → `downscaleKCentroid` to the detected (or `--scale`-overridden) native size → optional `--palette <profile>` (`quantizeOklab`/`quantizeBitReduce`) or `--colors N|auto` (`quantizeKMeans`/`estimateColorCount`) color step. No network call.

**Validation gate** (`core/validate.js`): deterministic post-generation checks (blank-frame, palette conformance incl. the 0.65× outline-darkened variants, transparency sanity, cross-frame drift). The Node path (`src/node/generate.js`) runs it after every generation and retries once with `seed + 1000`; the browser flow does not call it yet. Alongside it, `generateSprite` also runs a separate **advisory-only** signal on the pre-downscale AI source: `detectPixelGrid` compared against the requested sprite size, logged as a `validation` run-log warning (`warning: "grid-size-mismatch"`) on high-confidence disagreement (>25% off on either axis) — it never fails validation or triggers the retry; the grid-detection signal is still being gathered, not yet trusted to gate on.
